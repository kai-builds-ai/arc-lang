...
export type IRInstr =
  | { op: "const"; dest: string; value: number | string | boolean | null }
  | { op: "load"; dest: string; name: string }
  | { op: "store"; name: string; src: string }
  | { op: "binop"; dest: string; operator: string; left: string; right: string }
  | { op: "unop"; dest: string; operator: string; operand: string }
  | { op: "call"; dest: string; fn: string; args: string[] }
  | { op: "toolcall"; dest: string; method: string; url: string; body?: string }
  | { op: "field"; dest: string; obj: string; prop: string }
  | { op: "index"; dest: string; obj: string; idx: string }
  | { op: "setfield"; obj: string; prop: string; src: string }
  | { op: "setindex"; obj: string; idx: string; src: string }
  | { op: "jump"; target: string }
  | { op: "branch"; cond: string; ifTrue: string; ifFalse: string }
  | { op: "phi"; dest: string; sources: { block: string; value: string }[] }
  | { op: "ret"; value?: string }
  | { op: "list"; dest: string; elements: string[] }
  | { op: "map"; dest: string; keys: string[]; values: string[] }
  | { op: "label"; name: string }
  | { op: "print"; value: string }
  | { op: "range"; dest: string; start: string; end: string }
  | { op: "nop" };
...
export class IRGenerator {
  ...
  private defineVar(name: string): string {
  ...
  private lowerStmt(stmt: AST.Stmt): void {
    switch (stmt.kind) {
      case "LetStmt": {
        ...
        if (typeof stmt.name === "string") {
          ...
          const dt = stmt.name as AST.DestructureTarget;
  ...
  private lowerExpr(expr: AST.Expr): string {
    switch (expr.kind) {
      case "IntLiteral":
      case "FloatLiteral": {
        const dest = this.temp();
  ...
  private lowerPattern(pattern: AST.Pattern, subject: string): string {
    switch (pattern.kind) {
  ...
  private bindPattern(pattern: AST.Pattern, subject: string): void {
...
export function generateIR(program: AST.Program): IRModule {
  return new IRGenerator().generateIR(program);
}

export function generateIRFromSource(source: string): IRModule {
