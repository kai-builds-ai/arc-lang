...
pub fn version_to_string(v: SemVer) -> str {
  ...
  if v.prerelease != nil { "{base}-{v.prerelease}" } el { base }
}
...
pub fn resolve(registry: Registry, name: str, range: str) -> SemVer? {
  ...
  if range == "latest" {
    ...
    if latest_str != nil { ret parse_version(latest_str) }
  }
  ...
  if len(matching) > 0 { matching[0].version } el { nil }
}
...
fn build_dep_tree_recursive(registry: Registry, name: str, range: str, visited: mut map, depth: int) -> DepNode? {
  ...
  if visited[version_key] != nil {
    # Already resolved — ret cached
...
pub fn visualize_tree(node: DepNode, prefix: str, is_last: bool) -> str {
  let connector = if prefix == "" { "" } el if is_last { "└── " } el { "├── " }
  let extension = if prefix == "" { "" } el if is_last { "    " } el { "│   " }
...
pub fn download(registry: mut Registry, name: str, version_str: str) -> map {
  let pkg = registry.packages[name]
  if pkg == nil { panic("Package '{name}' not found") }
  ...
  if pkg_version == nil { panic("Version {version_str} not found for '{name}'") }
