...
const BINOPS = ["+", "-", "*", "/", "%", "**", "==", "!=", "<", ">", "<=", ">=", "++"];
const UNARYOPS = ["-", "not "];
function genIdent() { return pick(IDENTS); }
...
function genPipeline(depth) {
    const stages = randInt(2, 4);
    return Array.from({ length: stages }, () => genExpr(depth)).join(" |> ");
}
function genStringInterp(depth) {
    const parts = [];
    const count = randInt(1, 3);
    for (let i = 0; i < count; i++) {
        if (maybe())
            parts.push(pick(["hello", "world", " ", "test"]));
...
function mutateRemoveToken(src) {
    try {
        const tokens = lex(src);
        if (tokens.length < 3)
            return src;
...
function mutateDuplicateToken(src) {
    try {
        const tokens = lex(src);
        if (tokens.length < 2)
            return src;
...
function isCrash(e) {
    ...
    if (e instanceof Error) {
        ...
        if (msg.startsWith("Type error"))
            return false;
        if (msg.startsWith("Division by zero"))
            return false;
        ...
        if (msg.startsWith("Index"))
            return false;
        if (msg.startsWith("Stack"))
            return false;
        if (msg.startsWith("Max"))
            return false;
        if (msg.startsWith("Range"))
            return false;
        if (msg.startsWith("No matching"))
            return false;
        ...
        if (msg.includes("not supported"))
            return false;
        if (msg.includes("already defined"))
            return false;
...
function testProgram(source, result) {
    // Phase 1: Lexer
    let tokens;
    try {
        tokens = lex(source);
...
export function runFuzzer(iterations = 1000) {
    const result = {
        iterations,
        crashes: [],
        lexerErrors: 0,
...
export function fuzzReport(result) {
    console.log(`\n=== Fuzz Report ===`);
