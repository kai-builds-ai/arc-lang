...
function runBench(name, category, fn, opts = {}) {
...
function comparisonBenchmarks() {
    ...
    for (const cmp of comparisons) {
        ...
        // warmup
        for (let i = 0; i < 3; i++)
            interpretWithEnv(arcAst, createEnv());
        const arcStart = performance.now();
        for (let i = 0; i < cmp.iterations; i++)
            interpretWithEnv(arcAst, createEnv());
...
function countASTNodes(node) {
    ...
    for (const val of Object.values(node)) {
        if (Array.isArray(val)) {
            count += val.reduce((a, v) => a + countASTNodes(v), 0);
        }
