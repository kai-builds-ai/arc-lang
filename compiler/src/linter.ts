...
export function lint(source: string, options?: Partial<LintOptions>): LintDiagnostic[] {
  const opts = { ...DEFAULT_OPTIONS, ...options };
  const diagnostics: LintDiagnostic[] = [];

  function warn(rule: string, message: string, loc: AST.Loc) {
    diagnostics.push({ severity: "warning", rule, message, file: opts.file, line: loc.line, col: loc.col });
  }

  function info(rule: string, message: string, loc: AST.Loc) {
  ...
  interface VarInfo {
    ...
    used: boolean;
    mutated: boolean;
    kind: "variable" | "function" | "parameter" | "import" | "type" | "loop-var" | "destructured";
  }

  class LintScope {
    vars = new Map<string, VarInfo>();
    children: LintScope[] = [];
    constructor(public parent?: LintScope) {}
    ...
    lookupAll(name: string): VarInfo | undefined {
      return this.vars.get(name) ?? this.parent?.lookupAll(name);
    }

    markUsed(name: string) {
      const v = this.vars.get(name);
      if (v) { v.used = true; return; }
      this.parent?.markUsed(name);
    }

    markMutated(name: string) {
      const v = this.vars.get(name);
      if (v) { v.mutated = true; return; }
  ...
  function checkVarNaming(name: string, loc: AST.Loc) {
    if (name === "_") return;
    if (!isSnakeCase(name)) {
  ...
  function checkFnNaming(name: string, loc: AST.Loc) {
    if (!isSnakeCase(name)) {
      info("naming-convention", `Function '${name}' should use snake_case`, loc);
    }
  }

  function checkTypeNaming(name: string, loc: AST.Loc) {
    if (!isPascalCase(name)) {
      info("naming-convention", `Type '${name}' should use PascalCase`, loc);
    }
  }

  function analyzeExpr(expr: AST.Expr, scope: LintScope): void {
    switch (expr.kind) {
      case "IntLiteral":
      case "FloatLiteral":
      case "BoolLiteral":
      ...
      case "StringInterp":
        for (const part of expr.parts) {
          if (typeof part !== "string") analyzeExpr(part, scope);
        }
        break;
      case "Identifier":
      ...
      case "CallExpr":
        ...
        for (const arg of expr.args) analyzeExpr(arg, scope);
        break;
      case "MemberExpr":
      ...
      case "MapLiteral":
        for (const entry of expr.entries) {
          if (entry.spread) { analyzeExpr(entry.spread, scope); continue; }
          if (entry.key && typeof entry.key !== "string") analyzeExpr(entry.key as AST.Expr, scope);
          if (entry.value) analyzeExpr(entry.value, scope);
        }
        break;
  ...
  function analyzePattern(pat: AST.Pattern, scope: LintScope): void {
    if (pat.kind === "BindingPattern") {
      ...
      for (const el of pat.elements) analyzePattern(el, scope);
    } else if (pat.kind === "OrPattern") {
      for (const p of pat.patterns) analyzePattern(p, scope);
    }
  }

  function hasReturnInBlock(stmts: AST.Stmt[]): number {
  ...
  function analyzeStmts(stmts: AST.Stmt[], scope: LintScope, isTopLevel = false): void {
  ...
  function analyzeStmt(stmt: AST.Stmt, scope: LintScope, isTopLevel = false): void {
    switch (stmt.kind) {
      case "LetStmt": {
